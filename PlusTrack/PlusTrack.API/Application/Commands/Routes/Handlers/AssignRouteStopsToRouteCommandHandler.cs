...
namespace PlusTrack.API.Application.Commands.Routes.Handlers
{
    public class AssignRouteStopsToRouteCommandHandler : IRequestHandler<AssignRouteStopsToRouteCommand, List<RouteStop>>
    {
        ...
        public async Task<List<RouteStop>> Handle(AssignRouteStopsToRouteCommand request, CancellationToken cancellationToken)
        {
            ...
            var routeStops = _context.RouteStops.Include(x => x.Package).Where(x => x.Package.Status == (int)PackageStatus.EntregaFallida && x.Package.CompanyId == request.CompanyId).Take(request.AmountOfRouteStops ?? 20).ToList();
            
            //! Add to those route stops some more untill fills the amount of route stops
            routeStops.AddRange(_context.RouteStops.Include(x => x.Package).Where(x => x.RouteId == null && x.Package.CompanyId == request.CompanyId).Take(request.AmountOfRouteStops - routeStops.Count ?? 20 - routeStops.Count).ToList());
